...
class HomeAmazingItemsViewModel:ViewModel() {

    val items= MutableLiveData<List<AmazingItems>>()
    val error= MutableLiveData<String>()
    val loading= MutableLiveData<Boolean>()
    fun getAllAmazingItems(){
        Log.e("3636","start ...")
        ...
        CoroutineScope(Dispatchers.IO).launch {
            ...
            withContext(Dispatchers.Main){
                try {
                    ...
                    val response=ApiClient.api.getAmazingProducts()
                    Log.e("3636",response.code().toString())

                    val gson=Gson()
                    if (response.isSuccessful && response.body()!=null){
                        response.body()!!["data"]?.let { allitems->
                            items.value= gson.fromJson(allitems, Array<AmazingItems>::class.java).asList()
                            loading.value=false
                        }
                    }else{
