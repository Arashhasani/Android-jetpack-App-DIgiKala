...
class HomeSliderViewModel:ViewModel() {

    val slides= MutableLiveData<List<Slides>>()
        ...
        CoroutineScope(Dispatchers.IO).launch {
            ...
            withContext(Dispatchers.Main){
                try {
                    ...
                    val response=ApiClient.api.getAllSlides()
                    Log.e("3636",response.code().toString())

                    val gson=Gson()
                    if (response.isSuccessful && response.body()!=null){
                        response.body()!!["data"]?.let { allSlides->
                            slides.value= gson.fromJson(allSlides, Array<Slides>::class.java).asList()
                            loading.value=false
                        }
                    }else{
